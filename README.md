...
- 居民档案：登记、查询、更新和关联事务。
- 居民导入：村级数据分阶段入库——批次暂存并完成字段、证件唯一性与批内关联校验，审核人员查看差异后裁决覆盖或跳过，再由另一名有权限人员确认；确认前不改变正式居民数据，确认时整批写入、失败完整回滚。相同文件与参数重复提交返回原批次，修正后形成新版本；确认期间出现的居民冲突会重新标记待处理。失败行、覆盖决定和最终写入均可通过 `/api/imports` 查询，导入进度持久化在 SQLite 中，重启不丢失。
...
测试覆盖身份初始化、登录、用户与角色维护、权限计算、账号停用后的会话撤销、审计脱敏、居民事务、信访状态流转、公告排序、后台任务去重与领取、数据库时间格式，以及居民导入的批次校验、幂等重提、版本取代、双人确认、冲突重标记、原子回滚和重启恢复。
